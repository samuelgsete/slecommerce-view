...
@Injectable()
export class RecursoAdicionarUnidadeItem extends RecursoBase<Carrinho> {
    ...
    public executar(carrinhoId: number, itemAdicionado: ItemCarrinho): void {
        ...
        this.editar.executar(carrinhoId, itemAdicionado).subscribe(response => {
            ...
            this.toastr.error('Não foi possivel adicionar a unidade ao item', 'Poooxa!', { progressBar: true })
        }).add(() => {
            this.spinner.hide()
        })
    }
